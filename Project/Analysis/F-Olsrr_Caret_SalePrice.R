...
train_test <- train[x,]
train_train <- train[-x,]
train_testlogprice <- train_test$logsaleprice
...
predictions_b$SalePrice[is.na(predictions_b$SalePrice)] <- 150000
# Write CSV in R
write.csv(predictions_b , file = "Result_b_29.csv", row.names = FALSE)
